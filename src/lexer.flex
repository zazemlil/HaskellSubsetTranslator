...
\'[^']\'           { yylval->emplace<char>(std::string(yytext + 1, yyleng - 2)[0]); return Parser::token::T_LITERAL_CHAR; }
...
std::tuple<syntax_tree::AST, syntax_tree::AST> analyze(char* arg, bool printError) {
    yyscan_t scanner;
    yylex_init(&scanner);
    
    set_input_file(scanner, arg);
    
    syntax_tree::AST ast, dataDeclarations = syntax_tree::AST(std::make_shared<syntax_tree::ASTNode>("DATA_DECLARATIONS"));;
    haskell_subset::Parser parser{ scanner, ast, dataDeclarations, printError };
    parser.parse();
    
    yylex_destroy(scanner);

    return std::make_tuple(ast, dataDeclarations);
}